...
//
//   import { mountScatter } from '/site/assets/bench-scatter.js';
...
export function mountScatter(section, { series, axes, metric = 'cost' }) {
  ...
  const render = () => {
    ...
    for (const s of series) {
      ...
      for (const p of pts) {
        // the hit disc carries focus, name and handlers; the dot is paint only
        // (pointer-events: none in bench.css). 16 units is 28px on a phone's
        // 680px chart, 27px on a landscape phone's 654 and 32 on the desktop's
        // 760: past WCAG 2.2's 24px floor at every width the matrix renders.
        const c = el('circle', {
          class: 'hit', cx: sx(p[metric]), cy: sy(p.score), r: 16, tabindex: 0, role: 'button',
          'aria-label': `${s.name}, ${p.effort}: ${p.score.toFixed(1)}% at ${ax.fmt(p[metric])} per task`,
        });
        const dot = el('circle', { class: 'dot', cx: sx(p[metric]), cy: sy(p.score), r: s.us ? 6 : 4.2, 'aria-hidden': 'true' });
        const show = () => {
        ...
        // a finger has no hover: the tap focuses the disc (show) and the tip
        // stays until focus moves — pointerleave, which a touch fires on
        // lift, only hides for a mouse
