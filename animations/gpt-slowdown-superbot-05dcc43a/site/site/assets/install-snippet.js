...
//   import { mountInstallSnippet } from './install-snippet.js';
...
// ./install-snippet.js — the name and shape are fixed.
...
.sb-oneliner .sb-cmd:focus-within { border-color: var(--accent, #2b6bff); }
.sb-oneliner .sb-cmd.copied {
  border-color: var(--accent, #2b6bff);
  box-shadow: 0 0 0 4px rgb(43 107 255 / 0.18), 0 0 24px rgb(43 107 255 / 0.2);
}
.sb-oneliner .sb-cmd code {
  flex: 1; overflow-x: auto; white-space: nowrap; color: var(--fg, #ececec); scrollbar-width: none;
...
.sb-oneliner .sb-cmd .sb-copy-alt:hover { border-color: var(--accent, #2b6bff); filter: none; }
.sb-oneliner .sb-cmd button.copied { background: linear-gradient(135deg, #2b6bff 0%, #6a1fd8 55%, #c026d3 100%); color: #ffffff; border-color: transparent; }
...
.sb-oneliner a.sb-deep:hover { border-color: var(--accent, #2b6bff); }
...
.sb-oneliner .sb-cmd code .sb-url { color: var(--accent, #2b6bff); }
...
.sb-oneliner details.sb-fold > summary::before { content: '▸'; color: var(--accent, #2b6bff); width: 1.2em; display: inline-block; transition: transform var(--dur-3, 200ms) var(--ease-out, ease); }
...
/* the fold's explanations are sentences: on a touch device they read at
   ...
   The command itself WRAPS on touch: a 393px phone showed the URL clipped
   with the rest in a scroll with its bar hidden (reviewer, 2026-09-02),
   which breaks the widget's own rule that the text shown is the text copied. */
@media (pointer: coarse) {
  .sb-oneliner .sb-what { font-size: var(--fs-sm, 14px); line-height: var(--lh-sm, 20px); }
  .sb-oneliner .sb-cmd code { white-space: normal; overflow-wrap: anywhere; overflow-x: visible; }
  .sb-oneliner .sb-cmd code.overflow { -webkit-mask-image: none; mask-image: none; }
  .sb-oneliner .sb-cmd { align-items: flex-start; }
  .sb-oneliner .sb-cmd .sb-copy, .sb-oneliner .sb-cmd .sb-copy-alt { align-self: center; }
}
...
.sb-oneliner .sb-dl-btn:focus-visible { outline: 2px solid var(--accent, #2b6bff); outline-offset: 2px; }
`;
...
export async function mountInstallSnippet(el, opts = {}) {
  ...
  const moreRow = () =>
    `<div class="sb-row sb-more"><a class="sb-more-link" href="${eo}/install">setup for every client</a></div>`;
  const fold = {
...
export function mountCommandRow(el, { cmd, copyLabel } = {}) {
  ...
  // overflow fade, same measured rule as the main row
  const measure = () => code.classList.toggle('overflow', code.scrollWidth > code.clientWidth + 1);
  measure();
  if (typeof ResizeObserver === 'function') new ResizeObserver(measure).observe(el);
}
